  ...
  return (
    <div className="animate-fadeIn">
      ...
      {isAdmin ? (
        ...
        <div>
          {loading ? (
            <div className="p-12 text-center text-gray-400"><div className="animate-spin w-8 h-8 border-2 border-[#F25C54] border-t-transparent rounded-full mx-auto mb-3"></div>Memuat data...</div>
          ) : members.length === 0 ? (
            <div className="p-12 text-center text-gray-400 animate-fadeIn"><Users className="w-12 h-12 mx-auto mb-3 opacity-30" />Data profil tidak ditemukan.</div>
          ) : (
            members.map((member) => (
              <div key={member.id} className="space-y-6 animate-slideUp">
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow duration-300">
                  <div className="flex items-center gap-4 mb-6">
                    {member.foto_url ? (
                      <img src={`${BASE_URL}${member.foto_url}`} alt="" className="w-20 h-20 rounded-full object-cover shadow-md hover:scale-105 transition-transform duration-300" />
                    ) : (
                    ...
                    <div>
                      <h2 className="text-xl font-bold text-[#3D5170]">{member.name}</h2>
                      <p className="text-gray-500">{member.nis || '-'}</p>
                      <p className="text-xs text-blue-500">{member.user?.email || '-'}</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    {[['Nama Panggilan', member.nama_panggilan], ['Rayon', member.rayon_role?.name], ['Rombel', member.rombel], ['TTL', member.tempat_tanggal_lahir], ['Jenis Kelamin', member.jenis_kelamin === 'L' ? 'Laki-laki' : member.jenis_kelamin === 'P' ? 'Perempuan' : '-'], ['Hobi & Minat', member.hobi_minat], ['Kontak WA', member.kontak_wa], ['Instagram', member.instagram], ['LinkedIn', member.linkedin]].map(([label, value], i) => (
                      <ProfileRow key={i} label={label} value={value} delay={i * 30} />
                    ))}
                  </div>
                  <div className="mt-4 text-sm"><p className="text-gray-500 font-medium">Alamat:</p><p className="text-[#3D5170]">{member.alamat || '-'}</p></div>
                  <div className="mt-4 text-sm"><p className="text-gray-500 font-medium">Deskripsi:</p><p className="text-[#3D5170]">{member.deskripsi_profil || '-'}</p></div>
                  <div className="mt-6">
                    <button onClick={() => handleEdit(member)} className="flex items-center gap-2 px-4 py-2 bg-[#F25C54] text-white rounded-lg text-sm font-medium hover:bg-[#e04f47] hover:scale-105 active:scale-95 transition-all duration-300 shadow-md shadow-red-200">
                      <Pencil className="w-4 h-4" /> Edit Profil
                    </button>
                  </div>
                </div>
                {/* SERTIFIKAT */}
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow duration-300 animate-slideUp animation-delay-200">
                  <h3 className="text-lg font-bold text-[#3D5170] mb-4">📄 Sertifikat</h3>
                  {member.sertifikat_urls?.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                      {member.sertifikat_urls.map((url, index) => (
                        <div key={index} className="border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300 animate-scaleIn" style={{ animationDelay: `${index * 100}ms` }}>
                          {url.toLowerCase().endsWith('.pdf') ? (
                            <a href={`${BASE_URL}${url}`} target="_blank" rel="noopener noreferrer" className="block p-4 bg-red-50 hover:bg-red-100 transition">
                              <span className="text-3xl">📕</span>
                              <p className="text-sm font-medium text-red-700 truncate mt-2">{url.split('/').pop()}</p>
                            </a>
                          ) : (
                            <a href={`${BASE_URL}${url}`} target="_blank" rel="noopener noreferrer" className="block group">
                              <img src={`${BASE_URL}${url}`} alt="" className="w-full h-40 object-cover group-hover:scale-105 transition-transform duration-500" />
                            </a>
                          )}
                          <div className="px-3 py-2 bg-gray-50 flex justify-between items-center">
                            <span className="text-xs text-gray-400">#{index + 1}</span>
                            <a href={`${BASE_URL}${url}`} download className="text-xs text-[#F25C54] hover:underline">Download</a>
                          </div>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <div className="text-center py-8 text-gray-400"><p className="text-4xl mb-2">📭</p><p className="text-sm">Belum ada sertifikat</p></div>
                  )}
                </div>
              </div>
            ))
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fadeIn">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-scaleIn">
            ...
            <form onSubmit={handleSubmit} className="px-6 py-4 space-y-4">
              ...
              <div><label className="block text-sm font-medium text-[#3D5170] mb-1">Sertifikat <span className="text-xs text-gray-400 font-normal">(bisa pilih banyak file)</span></label><div className="mb-2 px-3 py-2 bg-blue-50 border border-blue-100 rounded-lg text-xs text-blue-600">💡 Format: JPG, PNG, atau PDF. Maks 5MB per file.</div><div className="relative group"><input type="file" accept=".jpg,.jpeg,.png,.pdf" multiple onChange={handleSertifikatChange} className="absolute inset-0 opacity-0 cursor-pointer z-10" /><div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center group-hover:border-[#F25C54] transition-all duration-300 cursor-pointer bg-gray-50/50"><Upload className="w-10 h-10 mx-auto text-gray-400 mb-2 group-hover:text-[#F25C54] transition-colors" /><p className="text-sm font-medium text-gray-600">Klik untuk upload sertifikat</p><p className="text-xs text-gray-400 mt-1">JPG, PNG, atau PDF</p>{formData.sertifikat_data.length > 0 && (<div className="mt-3 pt-3 border-t border-gray-200 animate-slideDown"><p className="text-xs font-semibold text-[#F25C54] mb-2">{formData.sertifikat_data.length} file dipilih:</p><div className="space-y-1 max-h-32 overflow-y-auto">{Array.from(formData.sertifikat_data).map((file, i) => (<div key={i} className="flex items-center gap-2 text-xs text-gray-600 bg-white rounded px-3 py-1.5 animate-fadeIn" style={{ animationDelay: `${i * 50}ms` }}><span className="text-green-500">✓</span><span className="truncate">{file.name}</span><span className="text-gray-400 flex-shrink-0">({(file.size / 1024).toFixed(1)} KB)</span></div>))}</div></div>)}</div></div></div>
